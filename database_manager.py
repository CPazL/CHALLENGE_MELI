...
# Función para conectar a la base de datos
    ...
    def crear_tablas(self,cursor):
        try :
            ...
            cursor.execute(f"CREATE TABLE {table_name_files_all} ( id_archivo VARCHAR(255)  PRIMARY KEY,nombre VARCHAR(255) NOT NULL, extension VARCHAR(10),
            owner VARCHAR(255), visibilidad ENUM('privado','publico') NOT NULL, ultima_modificacion DATETIME);")

            #El id será una combinacion entre ID_ARCHIVO mas fecha_actualizacion
            cursor.execute(f"""CREATE TABLE {table_name_files_public} (id varchar(255)  PRIMARY KEY,
            archivo_id INT, fecha_cambio DATETIME, FOREIGN KEY (archivo_id) REFERENCES {table_name_files_all}(id)); """)
    ...
    def elimina_usuario(self,cursor, access_user):
